...
[프로그램 2] 소단원 JSON 업로드 → 검증 → 저장 → 진행 갱신 → 다음 작업 안내
                    ...
                    ▼
[프로그램 2] 지금까지 그 챕터에 있는 소단원만으로 즉시 병합 → data/<slug>.json + manifest.json 갱신
                    │  (챕터가 다 갖춰지길 기다리지 않는다 — 첫 소단원만 있어도 그 순간부터 반영된다.
                    │   다 갖춰지면 같은 파일이 그대로 완성본이 될 뿐, 별도 '완료 단계'가 없다)
                    ▼
[Claude] dev 브랜치에 커밋·푸시 + 최신본을 사용자에게 전송
```
...
> **챕터가 끝나야 사이트에 보이는 게 아니다.** `submit`할 때마다 그 챕터에 지금까지 존재하는 소단원만
> 모아 `data/<slug>.json`을 그 자리에서 다시 쓰고 `manifest.json`에도 등록한다 — 챕터의 첫 소단원 하나만
> 완성돼도 그 즉시 사이트 드롭다운에 나타나고 그 한 소단원만큼만 열람할 수 있다. 이후 소단원을 추가로
> 제출할 때마다 같은 파일이 그만큼씩 갱신된다. 진행 상황은 `submit` 출력의 `[[PUBLISHED ... status=partial|merged]]`
> 마커로 구분한다 — `partial`은 아직 남은 소단원이 있다는 뜻이고 `merged`는 챕터가 완전히 끝났다는 뜻이다(둘 다
> Claude가 커밋·푸시해야 하는 신호).
...
- **순서 경고**: 위 「순서 보장」 참고.
- **즉시 반영**: `submit`(및 `undo`) 할 때마다 그 챕터에 지금까지 있는 소단원만 이어붙여 그 자리에서
  `data/<slug>.json`으로 다시 쓰고 `data/manifest.json`(book 필드 포함)을 갱신합니다 — 챕터가 다 갖춰지길
  기다리지 않습니다. 별도의 최종복습 JSON은 없다.
- 출력의 `[[PUBLISHED ... status=partial|merged]]` 마커는 Claude가 다음 조치(커밋·전송)를 판단하는 신호입니다.
  `partial`은 그 챕터가 아직 진행 중(일부 소단원만 반영), `merged`는 챕터가 완전히 끝났다는 뜻입니다.
